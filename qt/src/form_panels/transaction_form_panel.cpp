...
TransactionFormPanel::TransactionFormPanel(QWidget* parent) : QWidget(parent)
{
	...
	pPayeeLayout->addWidget(pPayeeLabel);
	pPayeeLayout->addWidget(m_pPayee);
	
	QHBoxLayout* pAmountLayout = new QHBoxLayout();
	...
	pAmountLayout->addStretch();
	pAmountLayout->addWidget(pAmountLabel);
	pAmountLayout->addWidget(m_pAmount);
	
	
	QHBoxLayout* pCategoryLayout = new QHBoxLayout();
	...
	pCategoryLayout->addWidget(pCategoryLabel);
	pCategoryLayout->addWidget(m_pCategory);
...
//	pGridLayout->addLayout(pPayeeLayout, 1, 1, 1, 1);
//	pGridLayout->addLayout(pAmountLayout, 1, 2, 1, 1);
//	pGridLayout->addLayout(pCategoryLayout, 2, 1, 1, 1);
	...
	pGridLayout->addWidget(m_pDescription, 3, 1, 1, 4, 0);
...
void TransactionFormPanel::setFocusPayee()
{
	m_pPayee->setFocus();
}
